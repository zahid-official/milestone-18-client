...
const scopeOptions = ["GLOBAL", "VENDOR"].map((scope) => ({
  value: scope,
  label: scope
    .split("_")
    .map((part) => part.charAt(0) + part.slice(1).toLowerCase())
...
// CouponManagementPage Component
const CouponManagementPage = async ({ searchParams }: IProps) => {
  const params = (await searchParams) || {};
  const queryString = queryFormatter(params);

  const userInfo = await getUserInfo();
  const currentUserId = userInfo?._id ?? null;
  const currentUserRole = userInfo?.role ?? null;
